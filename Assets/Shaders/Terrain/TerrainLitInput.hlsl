...
void TerrainInstancing(inout float4 positionOS, inout float3 normal, inout float2 uv)
{
    ...
    float3 distanceToCamera = (worldPosition.xyz) - _WorldSpaceCameraPos;

    float distance = sqrt(pow(distanceToCamera.x, 2.0) + pow(distanceToCamera.z, 2.0));

    float size = 40.0;
    float depth = 3;
    float horizonDistance = 600;
    float horizonSize = 30;

    float switchDistance = 400;

    // float offsetA = depth * (-pow(distance / size, 2));
    // float offsetB = depth * (-pow((distance - horizonDistance) / horizonSize, 2)) - 30;

    float yOffset = 0;

    if (distance <= switchDistance)
    {
        yOffset = depth * (-pow(distance / size, 2));
    }
    else if (distance < (horizonDistance))
    {
        yOffset = depth * (-pow((distance - horizonDistance) / horizonSize, 2)) - 30;
    }
    else
    {
        yOffset = -30;
    ...
    positionOS = mul(unity_WorldToObject, worldPosition);
